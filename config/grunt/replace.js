...
const replaceHashInMatch = (grunt, match, prefix, index) => {
    const filename = grunt.file.expand({ cwd: 'build/web-audio-conference-2018/scripts' }, `${ prefix }.*.js`)[0];

    if (filename === undefined) {
        return match;
    }

    const hash = `sha384-${ computeHashOfFile(`build/web-audio-conference-2018/scripts/${ filename }`, 'sha384', 'base64') }`;
...
module.exports = (grunt) => {
    return {
        'bundle': {
            ...
            options: {
                patterns: [ {
                    ...
                    replacement: '"/web-audio-conference-2018/ngsw-worker.js"'
                } ]
            }
        },
        'chunks': {
            files: {
                './': [
                    'build/web-audio-conference-2018/index.html'
                ]
            },
            options: {
                patterns: [ {
        ...
        'csp-production': {
            ...
            options: {
                patterns: [ {
                    ...
                    replacement: () => {
                        const html = fs.readFileSync('build/web-audio-conference-2018/index.html', 'utf-8'); // eslint-disable-line node/no-sync
        ...
        'manifest': {
            files: {
                './': [
                    'build/web-audio-conference-2018/ngsw.json'
                ]
            },
            options: {
                patterns: [ {
                    match: /assets\/(?<filename>[\da-z-]+)\.(?<extension>ico|jpg|png)/g,
                    replacement: (_, filename, extension) => grunt.file.expand({ cwd: 'build/web-audio-conference-2018', ext: extension }, `assets/${ filename }.*`)[0]
                }, {
                    ...
                    match: /\s*"\/web-audio-conference-2018(?:\/scripts)?\/runtime(?:-es(?:2015|5))?.[\da-z]*\.js",/g,
                    replacement: ''
                }, {
                    match: /\s*"\/web-audio-conference-2018(?:\/scripts)?\/runtime(?:-es(?:2015|5))?.[\da-z]*\.js":\s"[\da-z]+",/g,
                    replacement: ''
                }, {
                    // Replace the hash value inside of the hashTable for "/scripts/*.js" because it may have been modified before.
                    match: /"\/web-audio-conference-2018(?<filename>\/scripts\/(?:\d+|main|scripts)(?:-es(?:2015|5))?.[\da-z]+.js)":\s"[\da-z]+"/g,
                    replacement: (_, filename) => {
                        return `"/web-audio-conference-2018${ filename }": "${ computeHashOfFile(`build/web-audio-conference-2018${ filename }`, 'sha1', 'hex') }"`;
                    }
                }, {
                    // Replace the hash value inside of the hashTable for "/styles/styles*.css" because it was modified before.
                    match: /"\/web-audio-conference-2018(?<filename>\/styles\/styles\.[\da-z]*\.css)":\s"[\da-z]+"/g,
                    replacement: (_, filename) => {
                        return `"/web-audio-conference-2018${ filename }": "${ computeHashOfFile(`build/web-audio-conference-2018${ filename }`, 'sha1', 'hex') }"`;
                    }
                }, {
                    // Replace the hash value inside of the hashTable for "/(index|start).html" because it was modified before.
                    match: /"\/web-audio-conference-2018\/(?<filename>index|start)\.html":\s"[\da-z]+"/g,
                    replacement: (_, filename) => {
                        return `"/web-audio-conference-2018/${ filename }.html": "${ computeHashOfFile(`build/web-audio-conference-2018/${ filename }.html`, 'sha1', 'hex') }"`;
                    }
                } ]
            }
        },
                ...
                patterns: [ {
                    ...
                    replacement: (match, filename, moduleAttribute) => {
                        if (moduleAttribute === undefined) {
                            return `<script>${ fs.readFileSync(`build/web-audio-conference-2018/${ filename }`) }</script>`; // eslint-disable-line node/no-sync
                        }

                        return `<script${ moduleAttribute }>${ fs.readFileSync(`build/web-audio-conference-2018/${ filename }`) }</script>`; // eslint-disable-line node/no-sync
                    }
                } ]
            }
        },
                ...
                patterns: [ {
                    ...
                    replacement: (match, filename, moduleAttribute, initialHash) => {
                        const updatedHash = (/main(?:-es(?:2015|5))?\.[\da-z]+\.js/.test(filename)) ?
                            `sha384-${ computeHashOfFile(`build/web-audio-conference-2018/scripts/${ filename }`, 'sha384', 'base64') }` :
                            initialHash;
            ...
            options: {
                patterns: [ {
                    ...
                    replacement: (match, filename) => {
                        const hash = `sha384-${ computeHashOfFile(`build/web-audio-conference-2018/styles/${ filename }`, 'sha384', 'base64') }`;
