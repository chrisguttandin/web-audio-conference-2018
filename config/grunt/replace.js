...
module.exports = (grunt) => {
    return {
        ...
        'csp-production': {
            ...
            options: {
                patterns: [ {
                    ...
                    replacement: () => {
                        const html = fs.readFileSync('build/web-audio-conference-2018/index.html', 'utf-8');
                        const regex = /<script[^>]*?>([^<](.|[\n\r])*?)<\/script>/gm;
                        const scriptHashes = [];
        ...
        'fix': {
            files: {
                './': [
                    'build/web-audio-conference-2018/ngsw-worker.js'
                ]
            },
            options: {
                /*
        ...
        'manifest': {
            ...
            options: {
                patterns: [ {
                    ...
                    replacement: (_, filename, extension) => grunt.file.expand({ cwd: 'build/web-audio-conference-2018' }, `assets/*.${ filename }.${ extension }`)[0]
                }, {
                    ...
                    match: /[\s]*"\/web-audio-conference-2018(\/scripts)?\/runtime(?:-es(?:2015|5))?.[a-z0-9]*\.js",/g,
                    replacement: ''
                }, {
                    match: /[\s]*"\/web-audio-conference-2018(\/scripts)?\/runtime(?:-es(?:2015|5))?.[a-z0-9]*\.js":\s"[a-z0-9]+",/g,
                    replacement: ''
                }, {
                    // Replace the hash value inside of the hashTable for "/scripts/main-es*.js" because it was modified before.
                    match: /"\/web-audio-conference-2018(\/scripts\/main(?:-es(?:2015|5))?.[a-z0-9]+.js)":\s"[a-z0-9]+"/g,
                    ...
                    match: /"\/web-audio-conference-2018(\/styles\/styles\.[a-z0-9]*\.css)":\s"[a-z0-9]+"/g,
                    replacement: (_, filename) => {
                        return `"/web-audio-conference-2018${ filename }": "${ computeHashOfFile(`build/web-audio-conference-2018${ filename }`, 'sha1', 'hex') }"`;
                    }
                }, {
                    // Replace the hash value inside of the hashTable for "/(index|start).html" because it was modified before.
                    match: /"\/web-audio-conference-2018\/(index|start)\.html":\s"[a-z0-9]+"/g,
                    replacement: (_, filename) => {
                ...
                patterns: [ {
                    ...
                    replacement: (match, filename, moduleAttribute) => {
                        if (moduleAttribute === undefined) {
                            return `<script>${ fs.readFileSync(`build/web-audio-conference-2018/${ filename }`) }</script>`;
                        }

                        return `<script${ moduleAttribute }>${ fs.readFileSync(`build/web-audio-conference-2018/${ filename }`) }</script>`;
                    }
                } ]
            }
        },
